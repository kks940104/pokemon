...
// AuthenticationFailureHandler 실패 핸들러 설정 인터페이스
public class LoginFailureHandler implements AuthenticationFailureHandler  {
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        ...
        HttpSession session = request.getSession(); // request Session값
        RequestLogin form = Objects.requireNonNullElse((RequestLogin) session.getAttribute("requestLogin"), new RequestLogin());

        form.setErrorCodes(null); // List형식으로 남아있기에 null을 넣어 초기화 작업 진행.
        ...
        // region 아이디 또는 비밀번호를 입력하지 않은 경우, 아이디로 조회가 안되거나, 비번이 일치하지 않는 경우
