...
public class MessageDeleteService {
     ...
     * 1. sender 쪽에서 삭제하는 경우 / mode - send 일 경우
     *      deleteBySender 값을 true
     * 2. receiver 쪽에섯 삭제하는 경우 // mode - receive 일 경우
     *      deleteByReceiver 값을 true
     * 3. deleteBySender 와 deleteByReceiver 가 모두 true인 경우 실제 DB에서도 삭제 (Message 쪽 삭제, 파일 데이터 함께 삭제)
     * @param seq
     */
    public void process(Long seq, String mode) {
        ...
        if (item.isDeletedBySender() && item.isDeletedByReceiver()) {
            isProceedDelete = true; // 보낸쪽, 받는쪽 모두 삭제 한 경우 -> DB에서 삭제처리.
        }
