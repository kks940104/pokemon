window.addEventListener("DOMContentLoaded", function () {
  ...
  frmJoin.email.addEventListener("keyup", function () {
    if (this.value.trim()) {
  ...
  sendButton.addEventListener("click", function () {
    ...
    emailAuth.sendCode(email, updateTimer, function () {
      // 1. 전송버튼의 문구를 인증코드 재전송으로 변경, 이메일을 변경하지 못하게 처리
      //    인증 코드 인력 가능하게 처리
      //    인증하기 버튼 노출
  ...
  function updateTimer(seconds) {
    ...
    if (seconds > 0) {
      ...
      timerStr = `${("" + min).padStart(2, "0")}:${("" + sec).padStart(
        2,
        "0"
      )}`;
    } else {
      // 타이머가 0이 되면 다시 이메일 변경 가능하게 처리, 인증 코드 입력 불가 처리, 인증하기 버튼 감추기
      timerStr = "00:00";
