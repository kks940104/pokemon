...
function callbackFileUpload(files) {
    ...
    for (const {seq, location, fileName, fileUrl, selected} of files) {
        ...
        const selectImageEl = eq.querySelector(".select-image");
        ...
        if (location === 'editor') {
           imageUrls.push(fileUrl);
            ...
            if (selectImageEl) { // 리스트 노출 메인 이미지 선택 처리
                selectImageEl.addEventListener("click", () => selectImage(seq));
            }
